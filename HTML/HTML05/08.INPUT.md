...
```
```html
<!-- input type image는 이미지를 삽입할 뿐만 아니라 
아래와 같이 클릭하면 주소로 이동하는 제출 버튼으로도 활용할 수 있다. -->
<form action="/login">
    <input type="image" src="images/flower.png" alt="flower">
</form>

<!-- radio 버튼 셋중에 한가지 필수로 골라라 할때,
 같은 name 값 주면 세개가 나란히 나타나게된다. -->
 <input type="radio" name="radio11">
 <!-- checked 속성을 하면 해당 래디오 버튼에 체크되어 있다. -->
